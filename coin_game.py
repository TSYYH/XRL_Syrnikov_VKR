    ...
    def __init__(
        ...
        def _update_stats(
            ...
            coop1 = state.coop1 + jnp.zeros_like(
                state.counter, dtype=jnp.int16
            ).at[idx].set(jnp.ravel(jnp.asarray(rr))[0])
            coop2 = state.coop2 + jnp.zeros_like(
                state.counter, dtype=jnp.int16
            ).at[idx].set(jnp.ravel(jnp.asarray(bb))[0])
        ...
        def _step(
            ...
            ).at[state.outer_t].set(jnp.ravel(jnp.asarray(red_red_matches))[0])
            next_red_defect = state.red_defect + jnp.zeros(
                num_outer_steps, dtype=jnp.int8
            ).at[state.outer_t].set(jnp.ravel(jnp.asarray(red_blue_matches))[0])
            next_blue_coop = state.blue_coop + jnp.zeros(
                num_outer_steps, dtype=jnp.int8
            ).at[state.outer_t].set(jnp.ravel(jnp.asarray(blue_blue_matches))[0])
            next_blue_defect = state.blue_defect + jnp.zeros(
                num_outer_steps, dtype=jnp.int8
            ).at[state.outer_t].set(jnp.ravel(jnp.asarray(blue_red_matches))[0])
