...
def _normalize_tree_leaf(x):
    ...
    if x.ndim == 3 and x.shape[1] == x.shape[2]:
        return jnp.diagonal(x, axis1=1, axis2=2)
...
class RewardModeWrapper(JaxMARLWrapper):
    def __init__(self, env, mode="selfish", beta=0.5):
        super().__init__(env)
        self.mode = mode
        self.beta = beta
    ...
    def step(self, key, state, action):
        ...
        reward_vec = jnp.stack([reward[a] for a in self._env.agents], axis=0)
        team_reward = jnp.mean(reward_vec)

        if self.mode == "team":
            reward = {a: team_reward for a in self._env.agents}
        elif self.mode == "mixed":
            reward = {
                a: self.beta * reward[a] + (1.0 - self.beta) * team_reward
...
def _flatten_env_agent_tree(tree, num_actors):
    def _reshape_leaf(x):
        if not hasattr(x, "shape"):
            return x
        if x.ndim >= 2 and x.shape[0] * x.shape[1] == num_actors:
...
def make_train(config):
    ...
    env = RewardModeWrapper(
        env,
        mode=config.get("REWARD_MODE", "selfish"),
        beta=float(config.get("REWARD_BETA", 0.5)),
    )
    env = LogWrapper(env)
    ...
    def train(rng):
        ...
        def _update_step(update_runner_state, unused):
            ...
            def _env_step(runner_state, unused):
                ...
                obsv = jax.tree.map(_normalize_tree_leaf, obsv)
                env_state = jax.tree.map(_normalize_tree_leaf, env_state)
                reward = jax.tree.map(_normalize_tree_leaf, reward)
                done = jax.tree.map(_normalize_tree_leaf, done)
                info = jax.tree.map(_normalize_tree_leaf, info)

                info = jax.tree.map(
                    lambda x: (
                        x.reshape((config["NUM_ACTORS"],))
                        if hasattr(x, "size") and x.size == config["NUM_ACTORS"]
                        else (
                            x.reshape((config["NUM_ACTORS"],) + x.shape[2:])
                            if hasattr(x, "ndim") and x.ndim >= 2 and x.shape[0] * x.shape[1] == config["NUM_ACTORS"]
                            else x
                        )
                    ),
                    info,
        ...
        def _get_greedy_metrics(rng, actor_params):
            ...
            def _greedy_env_step(step_state, unused):
                ...
                info = _flatten_env_agent_tree(info, config["NUM_TEST_ACTORS"])
        ...
        rng, _rng = jax.random.split(rng)
        test_metrics = _get_greedy_metrics(_rng, actor_train_state.params) if config["TEST_DURING_TRAINING"] else {}  # initial greedy metrics
